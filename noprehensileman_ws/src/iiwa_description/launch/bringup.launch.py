...
def generate_launch_description():
    ...
    spawn_object = Node(
        ...
        arguments=[
            ...
            '-y', '0.0',  # Y coordinate
            '-z', '0.95',  # Z coordinate
            '-Y', '-0.16',  # Yaw (rotation around Z axis)-0.230568
            '-P', '-1.48',  # Pitch (rotation around Y axis)-1.424919
            '-R', '-3.0'   # Roll (rotation around X axis)-2.825518
        ],
        output='screen'
    )
    #_______________________________________________________________________
    ...
    return LaunchDescription([
        ...
        #                                on_start=[LogInfo(msg="______________ CAMERA SPAWN ______________"), 
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('apriltag_ros'), 'launch/tag_gazebo_1.launch.py')
            ),
        )
    ])
